...
**Purchase Order confirmation attribution — DONE, not yet committed.** Migration `005_purchase_order_confirmation_attribution.sql` applied to the live Supabase DB (`cbhllxodkfmtgfzeejka`, via Management API since the Supabase MCP server is `--read-only`) and schema-cache-reloaded; verified via `execute_sql` that the columns exist and the one pre-existing confirmed PO backfilled correctly as `'company'`. Code changes done in `src/pages/api/purchase-orders.ts` (stamp + audit row) and `src/pages/purchase-orders.astro` ("Confirm on behalf" button) — see "Purchase Order Confirmation Attribution" above. **Not yet committed/pushed** — working tree has these changes plus this doc update uncommitted as of end of session.
