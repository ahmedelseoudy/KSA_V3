...
**Recommended next steps:** continue Phase 2 on `/purchase-orders` using `v_batch_company_lifecycle`, grouped by batch with one row per company asked so silent companies remain visible. Add the server-side decision summaries (ready to request delivery, awaiting confirmation, overdue, delivered value/fill rate), but preserve `?id=&items=true` and every existing `po.company`, `po.availability_order.batch`, and stored-status field through a compatibility shim before touching the page. Regression-test `comparison.astro` plus both portal PO/delivery consumers against that shim. Only after those shapes are proven stable should `/deliveries` adopt the shared lifecycle/table shell and fix its full-quantity prefill safeguard.
