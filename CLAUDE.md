...
## Handoff / Open Items (as of 2026-07-29)
...
**v3.1 Phase 2 Purchase Orders + Deliveries slice — DONE and saved; not deployed.** Saved on local `main` in feature commit `8433994` (`feat: add purchase order and delivery lifecycle views`) followed by this handoff-documentation commit; the branch is two commits ahead of `origin/main`, and it was deliberately not pushed because a push may trigger Render deployment. The opt-in lifecycle API, grouped admin Purchase Orders page, grouped Deliveries page, compatibility assertions, and safe changed-only quantity editor are implemented. `npm run build`, `npx tsc --noEmit --skipLibCheck`, and `git diff --check` pass. An authenticated read-only smoke test against current data validated both API modes (legacy 4 POs; lifecycle 99 batch/company rows across 2 batches), and headless Chrome validated Purchase Orders and Deliveries on desktop/mobile with no runtime errors or page overflow. The delivery editor loaded five real items with zero values changed on load and zero pending items incorrectly prefilled to their maximum. The disposable fixture was extended to lock the legacy list/item and lifecycle response contracts, but could not be rerun because the local environment lacks `SUPABASE_SERVICE_ROLE_KEY`; it exited before creating data.

**Exact next-session sequence:**

1. Confirm `git status --short --branch` shows local `main` ahead of `origin/main` by two commits; ignore the unrelated untracked `supabase-mcp-setup.txt`.
2. Supply `SUPABASE_SERVICE_ROLE_KEY` to the test environment without committing it, then run `npm run test:e2e:lifecycle`. The fixture must finish with its database-confirmed cleanup checks green.
3. If the fixture passes, push `main` to deploy commit `8433994`.
4. After Render finishes, smoke-test `/purchase-orders`, `/deliveries`, `/comparison`, `/portal/purchase-orders`, and `/portal/deliveries`. In particular, confirm the portal and Comparison still receive the unchanged default list and `?id=&items=true` shapes.
5. Continue with lifecycle summaries in Analytics; add pagination/lazy loading if batch/company volume grows materially beyond the current 99 lifecycle rows.
