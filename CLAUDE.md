...
1. Finish the remaining PRD QA gates: measure a cold company portal page under one second, sweep anonymous `/api/*` denial, verify `available_qty > ordered` validation, and reconcile one fixture batch's raw `amazon_cost` sum with `/api/analytics`.
2. Exercise invite failure compensation and, optionally, the admin email Retry button with disposable identities. The signed failure path and retry guards are fixture-covered, so do not hard-bounce a real supplier mailbox merely for testing.
