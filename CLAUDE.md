...
**v3.1 Phase 0 correctness — DONE.** Migration `006_correctness_and_drift.sql` is applied to live Supabase and schema-cache-reloaded. Verified live: four real users appear with correct roles/statuses; a missing-profile update returns 404; product DELETE routing/validation works; PO `{dry_run:true}` against the current batch returns 51 explicit `not_responded` results, creates zero POs, and sends zero emails. The `availability_order_ids` route was separately verified without a caller-supplied `batch_id`, confirming that the server derives the effective batch correctly. `npm run build`, `npx tsc --noEmit --skipLibCheck`, and `git diff --check` pass. The raw `tsc` command without `--skipLibCheck` still fails on pre-existing Astro dependency declaration errors in `node_modules`. Full delivery timestamp regression testing is pending because the live database has no POs; no production data was seeded solely for that test.

**Recommended next steps:** (1) review and commit the Phase 0 working tree while preserving the unrelated untracked `supabase-mcp-setup.txt`; (2) push/deploy Phase 0 by itself; (3) smoke-test `/admin`, delete a disposable product through the UI, enter one availability response on behalf of a company, generate its PO, and verify a repeated generation reports `already_generated`; (4) test `delivered_at` stability once that disposable PO has an item; then (5) begin Phase 1, the per-batch availability-page redesign. Do not combine Phase 1 into the Phase 0 commit/deploy.
