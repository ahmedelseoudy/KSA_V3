...
## Handoff / Open Items (as of 2026-07-30)
...
**Email delivery tracking + controlled retries — DONE locally; migration live; deployment/config pending.** Migration `008_email_delivery_tracking.sql` is applied to live Supabase and the schema cache is reloaded. Every transactional email now creates one notification/Resend message per recipient, uses an idempotency key and `notification_id` tag, and stores the provider message ID. `/api/webhooks/resend` verifies the untouched raw body with Svix HMAC headers, rejects signatures older than five minutes, deduplicates event IDs, and records sent/delivered/delayed/failed/bounced/suppressed/complained outcomes. Admins get `/notifications` with global counts, filters, exact provider errors, and explicit-recipient retries limited to three with a one-minute cooldown. Retry context never stores setup tokens, temporary passwords, or rendered email HTML. The pure signature fixture, disposable live-schema webhook fixture (valid/replay/tampered signature plus database-confirmed cleanup), authenticated notification API/page smoke test, typecheck, build, diff check, and full disposable lifecycle regression pass. Production still needs the code deployed, a Resend webhook registered for the seven events at `https://ksa-v3.onrender.com/api/webhooks/resend`, and its unique signing secret added to Render as `RESEND_WEBHOOK_SECRET` before real delivery outcomes can update the CRM.

**Exact next-session sequence:**

1. Commit and push the email-delivery slice, then confirm Render deploys it.
2. Register the production Resend webhook for sent/delivered/delayed/failed/bounced/suppressed/complained and add its signing secret to Render as `RESEND_WEBHOOK_SECRET`.
3. Send one tracked test to `test@omarna.com`, confirm `/notifications` moves from sent to delivered, and exercise a disposable signed failure/retry if needed.
4. Finish the QA/performance pass: two-company RLS, malformed/zero-match XLSX, duplicate/interrupted upload handling, 5k-row ingestion, and full admin-to-company lifecycle regression.
5. Keep the unrelated untracked `supabase-mcp-setup.txt` out of application commits unless the user explicitly asks to add it.
