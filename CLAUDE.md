...
**SAR currency standardization + responsive KPI audit — DONE and pushed.** Commit `8d96752` is on `main`/`origin/main`. `src/utils/currency.ts` centralizes explicit `SAR`/`en-SA` formatting and SAR/legacy-dollar import parsing. Admin Orders, Products, Availability, Purchase Orders, Deliveries, Comparison, Analytics, both supplier portal PO/delivery surfaces, and PO email totals use the shared formatter. This is presentation/input parsing only; existing stored values were not converted because they already represent Saudi riyals. The longer currency prefix exposed cramped summary cards, so `StatCard.astro` now sizes values relative to its container and the Admin, Dashboard, Products, Availability, Comparison, Analytics, Purchase Orders, Deliveries, Orders modal, and supplier-portal KPI grids use safer progressive breakpoints. Validation passed with `npx tsc --noEmit --skipLibCheck`, `npm run build`, `git diff --check`, the full disposable lifecycle fixture, an authenticated currency sweep with no `$`/`USD` remnants or runtime errors, and 28 authenticated layout checks across seven principal pages at 320, 390, 768, and 1400 px with no clipped/overflowing KPI values. The push should trigger Render automatically, but production deployment status and post-deploy currency rendering have not yet been checked.

**Exact next-session sequence:**

1. Confirm Render deployed commit `8d96752`, then smoke-test SAR values and KPI-card layout on the production admin and company-portal surfaces.
2. Continue with email production readiness: verify a custom Resend sending domain, failed-notification visibility, and retry behavior.
3. Finish the QA/performance pass: two-company RLS, malformed/zero-match XLSX, duplicate/interrupted upload handling, 5k-row ingestion, and full admin-to-company lifecycle regression.
4. Keep the unrelated untracked `supabase-mcp-setup.txt` out of application commits unless the user explicitly asks to add it.
